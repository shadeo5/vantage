# Vantage — project map (working doc)

> This is the **internal working map** — the full folder guide, doc index, and dated status log for anyone building on Vantage. For the portfolio-facing overview, see **[README.md](README.md)**.

Vantage is a mobile app that gives a photographer a personal reason to shoot **today** — the right place, right light, matched to their gear. An inspiration engine, not a utility.
...
- **E4 Backend — foundation live.** Supabase (schema + RLS + anon auth), app↔cloud sync (`lib/sync.ts`), deployed edge function + cron. Plain-English map: `docs/BACKEND_FIELD_GUIDE.html`. (Precompute/cache B2–B3 still planned.) **Real events (B4) — groundwork started (Jul 16):** the golden-set sourcing strategy is written (`docs/engineering/EVENT_DATA_SOURCES.md` — multi-source thesis, per-city must-have acceptance test, Ticketmaster spine + CVB trust tier + ArcGIS permits) and the **first curated Atlanta event catalog** is built + date-verified (`content-pipeline/content/atlanta-events.json`, 12 marquee events, Opportunity-shaped). **✅ Events now wired into the nudge (app slice, Jul 25):** `lib/events.ts` + `components/ComingUp.tsx` — a LIVE event can **take the headline** (light×gear + a magnitude scarcity boost, demoting a co-located venue spot per the eclipse rule) with a "Happening" Why-signal, and a **"Coming up"** shelf surfaces the next ~2 months (tap to expand the shoot) so events pay off in the lead-up, not just day-of. Verified in-browser (simulated Sep-5 Dragon Con takeover + the live Coming-up shelf). **✅ Events moved into Postgres (app slice, Jul 25):** an `events` table + RLS (`supabase/schema.sql`), a seed generator (`content-pipeline/build-events-seed.mjs` → `events_seed.sql`, `npm run seed:events`), and a DB reader `lib/eventPack.ts` (`fetchEvents`, bundled `ATLANTA_EVENTS` fallback) that `App.tsx` loads per city — so events are **editable from the Supabase dashboard with no app release** (run the DDL + seed once to go live; bundled fallback until then). 178 tests. Still to come (slice 2): **push parity** — the nightly Edge Function reading the same `events` table so the buzz can headline an event.
...
6. **Events / relevancy (B4)** — ✅ **app slice shipped** (events wired into the nudge: takeover + "Coming up" shelf, `lib/events.ts`). Next (slice 2): **push parity** (events into the nudge Edge Function + a Postgres `events` table, retiring the bundle) and a **live source** — the open question is still whether Discover Atlanta / Creative Loafing expose a usable feed (see `EVENT_DATA_SOURCES.md`). Catalog freshness rule now documented (re-verify a season ~6wks out).
7. Small solo items: **G3** (gear-banner re-prompt) · a quiet-hours / frequency cap on the nudge · content-pipeline run. *(Done: E9 PH7 deploy + `cloudFactor` into the nudge Edge Function — the nightly push is now phase- and weather-honest, matching the app.)*
