...
1. **Conditions "shoot brief" (building now)** — the additive brief per `docs/engineering/SHOOT_BRIEF.html`: extend the forecast with precipitation, add a pure **gear-readiness** check (kit's fastest aperture vs. darkness → fast-glass / tripod / chase-light / silhouette), a **shot-type** rules table (conditions → what to shoot), then surface it under the pick. Fixes the overcast "quiet night → empty list" collapse. Slices 1→4; conditions re-rank (D1) deferred.
...
7. Small solo items: **G3** (gear-banner re-prompt) · a quiet-hours / frequency cap on the nudge · content-pipeline run · **port `cloudFactor` into the nudge Edge Function** so the nightly push is weather-honest too.
