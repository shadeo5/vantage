...
export function fastestAperture(cameraId: string, lensIds: string[]): number {
  const cam = getCamera(cameraId);
  const fs: number[] = [];
  if (cam.fixedLens) fs.push(cam.fixedLens.maxAperture);
  for (const id of lensIds) fs.push(getLens(id).maxAperture);
...
export function kitReadiness(cameraId: string, lensIds: string[], c: Conditions): Readiness {
...
export function shootBrief(windowStart: Date, spot: Spot, cameraId: string, lensIds: string[], forecast?: Forecast | null): ShootBrief {
  ...
  const rainWarning =
    ...
    : null;
  return { conditions, readiness: kitReadiness(cameraId, lensIds, conditions), shots: shotsFor(conditions), rainWarning };
}
