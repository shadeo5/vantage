...
7. Small solo items: **G3** (gear-banner re-prompt) · a quiet-hours / frequency cap on the nudge · content-pipeline run · **port `cloudFactor` into the nudge Edge Function** so the nightly push is weather-honest too · **E9 PH7** (phase-honest nightly push — same UTC day-roll / current-phase read as PH1, ported into the Edge Function).
