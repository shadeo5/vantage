...
// Mirror the current gear profile (+ timezone) up to the cloud. Best-effort. Lenses are
// generic descriptors (jsonb) — the push reads them directly (no catalog lookup).
export async function saveProfile(cameraId: string, lenses: LensSpec[]): Promise<void> {
  ...
  else console.log("[sync] profile saved:", cameraId, `${lenses.length} lenses`);
}
