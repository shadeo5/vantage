  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.row}>
        <Pressable onPress={onWent} accessibilityRole="button" accessibilityLabel={`Yes, I shot ${spotName}`} android_ripple={{ color: "rgba(26,20,8,0.12)" }} style={({ pressed }) => [styles.btn, styles.yes, pressed && { opacity: 0.9, transform: [{ scale: 0.98 }] }]}>
          <Text style={styles.yesText}>I shot it 📷</Text>
        </Pressable>
        <Pressable onPress={onSkipped} accessibilityRole="button" accessibilityLabel={`No, I didn't make it to ${spotName}`} android_ripple={{ color: "rgba(255,255,255,0.06)" }} style={({ pressed }) => [styles.btn, styles.no, pressed && { opacity: 0.6 }]}>
